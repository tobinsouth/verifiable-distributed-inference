...
class Worker:
    ...
    def shutdown(self):
        ...
        if self.conn_coordinator_thread is not None and threading.current_thread() != self.conn_coordinator_thread:
            conditional_print('[LOGIC] Joining coordinator thread')
            self.conn_coordinator_thread.join()

        if self.outbound_conn_worker_thread is not None and threading.current_thread() != self.outbound_conn_worker_thread:
            conditional_print('[LOGIC] Joining outbound worker thread')
            self.outbound_conn_worker_thread.join()

        if self.inbound_conn_worker_thread is not None and threading.current_thread() != self.inbound_conn_worker_thread:
